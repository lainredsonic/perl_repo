...
int qqwry_get(char *addr1, char *addr2, const char *ip, const char *qqwry_file_name){
//	FILE *wry_file = fopen("/root/github/perl_repo/QQWry.Dat", "r");
	FILE *wry_file = fopen(qqwry_file_name,"r");
	if (!wry_file){
		printf("can't open wry_file: %s\n", qqwry_file_name);
		return 1;
	}
	qqwry_get_location(addr1, addr2, ip, wry_file);
	fclose(wry_file);
	return 0;
}
